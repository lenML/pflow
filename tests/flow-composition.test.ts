...
describe("Flow Composition Tests with Node", () => {
  test("flow as node", async () => {
    ...
    expect(shared.data.current).toBe(30);
  });
  ...
  test("nested flow", async () => {
    ...
    wrapperFlow.setShared(shared);
    await wrapperFlow.run();

    expect(shared.data.current).toBe(32);
  });
  ...
  test("flow chaining flows", async () => {
    ...
    expect(shared.data.current).toBe(40);
  });
