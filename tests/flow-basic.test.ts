...
class NumberNode extends Node<SharedStorage, Record<string, unknown>> {
  ...
  async prep(): Promise<void> {
    this._shared.data.current = this.number;
  }
}
...
class AddNode extends Node<SharedStorage> {
  ...
  async prep(): Promise<void> {
    if (this._shared.data.current !== undefined) {
      this._shared.data.current += this.number;
    }
  }
}
...
class CheckPositiveNode extends Node<SharedStorage> {
  ...
  async post(): Promise<string> {
    if (
      this._shared.data.current !== undefined &&
      this._shared.data.current >= 0
    ) {
      return "positive";
    } else {
      return "negative";
...
class FlakyNode extends Node<SharedStorage> {
  ...
  async post(prepRes: any, execRes: any): Promise<string> {
    this._shared.data.execResult = execRes;
    return "default";
  }
}
...
class ExecNode extends Node<SharedStorage> {
  ...
  async prep(): Promise<number> {
    ...
    return this._shared.data.current || 0;
  }
  ...
  async post(prepRes: any, execRes: any): Promise<string> {
    this._shared.data.current = execRes;
    return "default";
  }
}
...
describe("Pocket Flow Tests with Node", () => {
  test("single number", async () => {
    ...
    await pipeline.run();
    expect(shared.data.current).toBe(5);
  });
  ...
  test("sequence with chaining", async () => {
    ...
    expect(shared.data.current).toBe(16);
  });
  ...
  test("branching positive", async () => {
    ...
    expect(shared.data.current).toBe(15);
  });
  ...
  test("negative branch", async () => {
    ...
    expect(shared.data.current).toBe(-25);
  });
  ...
  test("cycle until negative", async () => {
    ...
    expect(shared.data.current).toBe(-2);
  });
  ...
  test("retry functionality", async () => {
    ...
    expect(shared.data.execResult).toBe("Success on attempt 2");
  });
  ...
  test("retry with fallback", async () => {
    ...
    expect(shared.data.execResult).toBe("Fallback executed due to failure");
  });

  test("exec method processing", async () => {
    const shared: SharedStorage = new Shared({ current: 5 });
    ...
    expect(shared.data.current).toBe(-50);
  });
});
